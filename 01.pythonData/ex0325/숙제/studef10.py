from stumanage.student10 import Student
...
def stuDelete():
    print()
    print('[ 학생성적삭제 ]')
    searchNmae=input('삭제할 학생이름을 입력하세요.>> ')
    for i, stu in enumerate(stuSave):
        if stu == searchNmae:
            print('{}의 학생이 검색되었습니다.'.format(searchNmae))
            
            flag=input('정말 삭제하겠습니까? ')
            if flag =='y' or flag =='Y':
                del(stuSave[i])
                print('{} 학생이 삭제되었습니다.'.format(searchNmae))
            else:
                print('삭제가 취소되었습니다.')
