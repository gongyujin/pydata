...
# newline='' ==> 자동엔터키 생략, 인코딩 : utf-8-sig타입으로 저장 ==>엑셀파일 열었을때 안깨짐
...
# tr안에 td를 가져오기 위해 for문 실행
for row in data_rows:
    # 각 row마다 td를 가지고 옴. ;list 타입 1,13,13,13,13,13,1,1,1,13,...
    columns = row.find_all('td') 
    # td가 1개인 것은 skip, td가 13인 데이터만 가져옴
    if len(columns) <=1:
        continue
    # td가 13인것만 존재, 데이터 읽어오기
